...
def fasttext_not_vb_check(token, sent, sentences_tag, bef_existed):
        ...
        if len(fasttext_recoms) > 0:
            for word_g in tqdm(fasttext_recoms):
                for sent in sentences:
                    if word_g in sent:
                        vb_check = fasttext_not_vb_check(word_g, sent, sentences_tag, bef_existed)
                        if vb_check:
...
def seeds_check(seeds,sentences):
    to_run = True
    not_in_sents = list()
    for seed in seeds:
        if any(seed in sent for sent in sentences):
            pass
        else:
            not_in_sents.append(seed)
    if not_in_sents:
        print("Some seeds not existed in the corpus, please check again.")
        to_run = False
    return to_run,not_in_sents
...
if to_run:
    recommed_results, not_in_vocab, all_recommed_results = sim_filter(word2vec_path=word2vec_path, fasttext_path=fasttex_path, seeds=start_seeds,
                                                                      ...
                                                                      sim_level=0.46, freq_threshold=2, sentences=sentences,
