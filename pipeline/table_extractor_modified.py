...
def get_extraction_outcome(xml_path, save_path, config_path):
    ...
    for file_i in range(len(os.listdir(xml_path))):
        tables = None
        all_tables = []
        doi_ = xml_name[file_i].replace(".xml", "")
        doi = doi_.replace("-", "/", 1)
        ...
        if tables:
            ...
            if all_tables:
                for table in all_tables:
                    ...
                    sheet_id += 1
                del xls['Sheet']
                log_wp.excel_save(xls, save_path + '/' + str(doi_) + ".xlsx")
    return all_error_file, len(all_error_file)

# xml_path = r"C:\Users\win\Desktop\test\xml_parsing\xml"
# save_path = r"C:\Users\win\Desktop\test\pipeline\excel\elsevier"
# config_path = r"dictionary.ini"
# te = get_extraction_outcome(xml_path, save_path, config_path)
